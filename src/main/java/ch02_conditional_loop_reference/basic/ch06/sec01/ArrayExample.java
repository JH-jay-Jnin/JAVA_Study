...
public class ArrayExample {
        ...
        public static void main(String[] args) {
            ...
            // 1-2. 변수 대입
            num = 5;
            // 생성된 num이라는 변수 공간에 5를 대입한다.

            // 1-3. 변수 사용
            System.out.println("num에 저장된 값 : " + num);
            // num이 작성된 자리에 num에 저장된 값을 읽어와서 출력해준다.

            System.out.println("--------------------------------------");

            // 2-1. 배열 선언 => 자료형 뒤에 "[]"를 함께 넣어야 한다.
            int[] arr;
            // Stack 영역에 int[] (int배열) 자료형 공간을
            // 8byte 할당하고 arr이라는 이름을 부여함

            // 2-2. 배열 할당
            arr = new int[3];
            // new : " new 연산자"라고 하며
            // 이 친구가 의미하는 건 Heap 메모리 영역에 새로운 공간(배열)을 할당
            // => new 연산자 로 생성되는 친구들은 앞으로 모두 Heap 영역에 생긴다.

            //int[3] : int 자료형 변수 3개를 하나의 묶음으로 나타내는 배열이다.

            // 결론 : new int[3]는 Heap 영역에 int 3칸짜리 int[]을 생성(할당)

            // ** 생성된 int[ ]에는 시작 주소가 담긴다!! **

            // arr = new int[3];
            // int[ ] = int[ ] => 같은 자료형 (int 배열) 이기 때문에 대입 연산 가능!
            //                     == 그래서 주소 값을 담을 수 있음
            ...
            // 2-3. 배열 요소 값 대입 -> 10 20 30이 결과창에 나오도록
            // arr은 int[ ] 참조변수지만,
